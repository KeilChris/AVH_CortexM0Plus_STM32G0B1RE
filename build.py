...
def build(config, results):
    ...
    file = f"Platform-{timestamp()}.zip"
    logging.info(f"Archiving build output to {file}...")
    with ZipFile(file, "w") as archive:
        archive.write(f"Objects/Platform.axf")
        archive.write(f"Objects/Platform.axf.map")
        archive.write(f"Objects/Platform.{config.target}.clog")
...
def run(config, results):
    ...
    if not results[0].success:
        ...
        results[0].test_report.write(f"Platform-{ts}.xunit")
...
def run_cbuild(config):
    return ["bash", "-c", f"cbuild.sh Platform.{config.target}.cprj"]


@matrix_command(test_report=ConsoleReport()|CropReport("---\[ UNITY BEGIN \]---", '---\[ UNITY END \]---')|UnityReport())
def run_vht(config):
    return ["VHT_MPS2_Cortex-M0plus", "-q", "--stat", "--simlimit", 1, "-f", "vht_config.txt", "Objects/Platform.axf"]
